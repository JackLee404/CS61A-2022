    ...
    if typed == '' and reference == '':
        ...
        for i in range(min(len(typed_words), len(reference_words))):
            if typed_words[i] == reference_words[i]:
                sums += 1
        return sums / len(typed_words) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    res_lst = [len(word_list), limit]
    for i in range(len(word_list)):
        diff = diff_function(typed_word, word_list[i], limit)
        if  diff < res_lst[1]:
            res_lst = [i, diff]
        elif diff == res_lst[1] and i < res_lst[0]:
            res_lst = [i, diff]

    if res_lst[0] < len(word_list):
        return word_list[res_lst[0]]
    else:
        return typed_word
...
def sphinx_swaps(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if abs(len(start) - len(goal)) > limit or limit < 0:
        return limit + 1
    else:
...
def minimum_mewtations(start, goal, limit):
    ...
    if abs(len(start) - len(goal)) > limit or limit < 0:  # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        return limit + 1
        # END
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    sums = 0
    dict_usr = {'id': user_id, 'progress': 0}
    ...
    dict_usr['progress'] = sums / len(prompt) 
    upload(dict_usr)
    # print('ID:', dict_usr['id'], 'Progress:', dict_usr['progress'])
    return dict_usr['progress']
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    match_dict = {'words': words, 'times': []}
    for lst in times_per_player:
        match_dict['times'].append([])
        for i in range(len(words)):
            match_dict['times'][-1].append(lst[i + 1] - lst[i])
    return match_dict
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if match['times'][j][i] < match['times'][rel_lst[-1]][i]:
                rel_lst[-1] = j
        lst[rel_lst[-1]].append(match['words'][i])
