...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if last_leader == leader:
        ...
        message = f'Player {leader} takes the lead by {abs(diff)}'
    return leader, message
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    hef_score =  hefty_hogs(score, opponent_score)
    if hef_score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    hef_score =  hefty_hogs(score, opponent_score)
    score1 = score + hef_score
    if hog_pile(score1, opponent_score) != 0:
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score == 0 and opponent_score ==0:
        return hog_pile_strategy(score, opponent_score, threshold, num_rolls)
    else:
        for i in range(num_rolls):
            score_ = score
            score_ += take_turn(i, score, opponent_score)
            score_ += hog_pile(score_, opponent_score) 
            if score_ > GOAL_SCORE:
                return i
    return hog_pile_strategy(score, opponent_score, threshold, num_rolls)
